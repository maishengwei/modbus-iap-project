...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static uint8_t flashPageBackup[FLASH_PAGE_SIZE] = {0};
...
uint32_t FLASH_If_Write128Byte(uint16_t fileIndex, uint8_t *dataBuf, uint8_t dataLen){
    ...
    uint32_t flashPageIndex = (APPLICATION_ADDRESS - ADDR_FLASH_START) / FLASH_PAGE_SIZE \
        + fileIndex / packTotalNumInPage;
    /* backup flash to ram */
    memcpy(flashPageBackup, (uint8_t *)ADDR_FLASH_PAGE(flashPageIndex),\
        FLASH_PAGE_SIZE);
    /* locate package offset in a flash page */
    uint32_t packOffsetInPage = (fileIndex % packTotalNumInPage) \
        * IAP_PACKAGE_SIZE;
    /* update ram */
    memcpy((flashPageBackup + packOffsetInPage), dataBuf, dataLen);
